...
class ItineraryAgent(BaseAgent):
    ...
    async def run(self, request: TravelSearchRequest, activities: dict = None, hotels: dict = None) -> dict:
        nights = (request.return_date - request.departure_date).days if request.return_date else 7
        interests_str = ", ".join(request.interests) if request.interests else "general travel"

        activities_summary = ""
        if activities and activities.get("results"):
            top = activities["results"][:12]
            brief = [{"name": a.get("name"), "category": a.get("category"), "duration_hours": a.get("duration_hours"), "price_usd": a.get("price_usd"), "location": a.get("location")} for a in top]
